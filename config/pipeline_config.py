...
class FluxRefineConfig(BaseModel):
    ...
    prompt_composite_only: str = (
        "Professional automotive compositing pass. Make the pasted car look naturally "
        "photographed inside the current studio scene. Keep the same car, same camera "
        "angle, same scale, same paint color, and same body shape, but rebuild the "
        "integration quality. Remove the cutout look completely: no hard black rim, no "
        "grey boundary, no jagged segmentation, no halo, no fringing, and no old "
        "background residue around the silhouette. Smooth and antialias the car edge "
        "so it belongs to the floor and wall. Clean the glass and glossy paint: remove "
        "old outdoor trees, clouds, sky, buildings, street reflections, signs, and old "
        "scene objects from the hood, roof, doors, side glass, windshield, mirrors, "
        "chrome, and trim. Replace those reflections with natural studio reflections "
        "from the current background: dark wall, pale floor, soft vertical gradients, "
        "and clean panel highlights. Through the windows, show only the current studio "
        "background or neutral tinted glass, never the old source scene. Generate a "
        "visible soft natural contact shadow under all tires and a subtle lower-body "
        "grounding shadow, darkest at the tire contact patches and fading smoothly "
        "onto the studio floor. The final image should look like one real photograph, "
        "not a pasted cutout."
    )
    prompt_with_reference: str = (
        "Professional automotive compositing pass. Image one is the current car "
        "composite. Image two is the clean empty studio background plate and is the "
        "visual target for glass, reflections, and floor contact. Image three is the "
        "placement guide for silhouette, scale, and tire contact points. Image four is "
        "the source car reference for identity, paint color, wheels, mirrors, trim, "
        "badges, lights, grille, and small missing details. Make the car in image one "
        "look naturally photographed inside image two. Keep the same car, same camera "
        "angle, same scale, same paint color, and same body shape, but rebuild the "
        "integration quality. Remove the cutout look completely: no hard black rim, no "
        "grey boundary, no jagged segmentation, no halo, no fringing, and no old "
        "background residue around the silhouette. Smooth and antialias the car edge "
        "so it belongs to the floor and wall. Restore source-car details only when "
        "they are visible in image four and were damaged by segmentation. Clean the "
        "glass and glossy paint: remove old outdoor trees, clouds, sky, buildings, "
        "street reflections, signs, and old scene objects from the hood, roof, doors, "
        "side glass, windshield, mirrors, chrome, and trim. Replace those reflections "
        "with natural studio reflections from image two: dark wall, pale floor, soft "
        "vertical gradients, and clean panel highlights. Through the windows, show "
        "only image two's studio background or neutral tinted glass, never the old "
        "source scene. Generate a visible soft natural contact shadow under all tires "
        "and a subtle lower-body grounding shadow, darkest at the tire contact patches "
        "and fading smoothly onto the studio floor. The final image should look like "
        "one real photograph, not a pasted cutout."
    )
