...
class FluxRefineConfig(BaseModel):
    ...
    prompt_composite_only: str = (
        ...
        "environment. The rendered background visible in image one is the only target "
        "environment. Keep it locked outside the car edge and tire contact area. The "
        "car must remain exactly the same car: identical silhouette, dimensions, "
        "viewing angle, camera perspective, proportions, body shape, wheel size, wheel "
        "position, ride height, glass shape, badges, grille, lights, trim, license "
        "area, and model-name text. Do not squish, stretch, rotate, resize, repaint, "
        "recolor, redesign, smooth, simplify, or regenerate any part of the car. "
        "Preserve the original car paint color exactly as seen in image one. Clean the "
        "raw segmentation defects: jagged edges, hard grey boundary, matte outline, "
        "alpha halo, fringing, cutout stair steps, and old background bleed. There "
        "must be no visible boundary or grey outline around the car. Remove outdoor or "
        "old-scene reflections from paint and glass: no trees, clouds, sky patches, "
        "buildings, street, neighboring cars, showroom signs, ceiling grids, or old "
        "walls may remain on the hood, roof, doors, mirrors, chrome, glossy trim, or "
        "windows. Replace those reflections with broad studio reflections that match "
        "the current rendered background: dark wall panels, pale floor, soft vertical "
        "wall gradients, and clean rectangular studio highlights. For windows, keep "
        "the original tint, transparency, darkness, edge highlights, and reflective "
        "sheen, but the visible through-glass content must show the current rendered "
        "background behind/around the car or neutral dark glass. Do not leave old "
        "outdoor scenery in any window. Create a visible soft natural contact shadow "
        "under every tire and subtly under the lower body, darkest at the tire contact "
        "points and fading smoothly onto the studio floor. The car must not float; no "
        "shadow is incorrect. Final result: photorealistic automotive background "
        "replacement with clean glass, clean edges, no halo, no boundary, no pasted-on "
    ...
    prompt_with_reference: str = (
        ...
        "cross-polarized automotive reference photograph. Image one is the current "
        "composite: the original car is already placed into the rendered new "
        "environment. Image two is the clean empty rendered background plate; use it "
        "as the target environment for window see-through content and studio "
        "reflections. Image three is only the placement guide showing the car "
        "location, silhouette, scale, and tire contact points. Image four is the "
        "source car reference for identity, paint color, missing parts, glass, trim, "
        "mirrors, badges, wheels, lights, grille, and license area. The car from image "
        "four must remain exactly the same car in image one: identical silhouette, "
        ...
        "car. Restore small source-car parts only if they are present in image four "
        ...
        "seen in image four. Clean the raw segmentation defects: jagged edges, hard "
        "grey boundary, matte outline, alpha halo, fringing, cutout stair steps, and "
        "old background bleed. There must be no visible boundary or grey outline "
        "around the car. Only remove unwanted reflections and color contamination "
        ...
        "specular highlights, and realistic automotive gloss. Remove old outdoor or "
        "old-scene reflections from paint and glass: no trees, clouds, sky patches, "
        "buildings, street, neighboring cars, showroom signs, ceiling grids, or old "
        "walls may remain on the hood, roof, doors, mirrors, chrome, glossy trim, or "
        "windows. Replace those reflections with broad studio reflections from image "
        "two: dark wall panels, pale floor, soft vertical wall gradients, and clean "
        "rectangular studio highlights. For windows, keep the original tint, "
        "transparency, darkness, edge highlights, and natural reflective sheen, but "
        "the visible through-glass content must show the current rendered background "
        "from image two behind/around the car or neutral dark glass. Do not leave old "
        "outdoor scenery in any window. Create a visible soft natural contact shadow "
        "under every tire and subtly under the lower body, darkest at the tire contact "
        "points and fading smoothly onto the studio floor. The car must not float; no "
        "shadow is incorrect. Final result: photorealistic automotive background "
        "replacement with the original car locked in place, clean glass, clean edges, "
        "no halo, no boundary, no pasted-on look, no car deformation, and no dimension "
        "change."
    )
